...
def blurMetric(imagePath):
    ...
    hEdge = cv2.Sobel(img_blur, ddepth=cv2.CV_64F, dx=1,
                      dy=0, ksize=3, scale=1)  # X axis
    vEdge = cv2.Sobel(img_blur, ddepth=cv2.CV_64F, dx=0,
                      dy=1, ksize=3, scale=1)  # Y Axis
    hvEdge = cv2.Sobel(img_blur, ddepth=cv2.CV_64F, dx=1,
                       dy=1, ksize=3, scale=1)  # Both
